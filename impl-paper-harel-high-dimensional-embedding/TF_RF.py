...
class FR_Algorithm:
    def __init__(self, number_of_nodes=None, number_of_edges=None,
        ...
        self.pos = tf.Variable(tf.zeros(shape=[self.number_of_edges, 3]))
    ...
    def calculate_vertex_forces(self, initialgraph=None):  # graph, array-like, shape=(n_sample, n_component), n_component=2 or 3
        ...
        if self.factor_attract != 0:
            k_at = (1.0 / (self.factor_attract) * np.sqrt(
                (self.MAX_X_DIMENSION * self.MAX_Y_DIMENSION) / float(self.number_of_nodes)))
            # print("number of nodes:", self.number_of_nodes)
        else:
            k_at = 0
        k_rep = self.factor_repulsion * np.sqrt(
            (self.MAX_X_DIMENSION * self.MAX_Y_DIMENSION) / float(self.number_of_nodes))
            ...
            if k_rep != 0:
                for h in range(self.number_of_nodes):
                    if h != i:
                        ...
                        disp[i][1] -= ((delta_y / distance) * (-(
                        k_rep * k_rep) / distance))  # debug! if delta_y replace with delta_x, and it will always draw strainght line.
...
if __name__ == "__main__":
    ...
    graph = FR.get_graph_from_file("data/tree_06_03.csv")
    ...
    FR.plot(graph=graph, transformed_points=new_coords, filename="FR_figure.png")
